#!/usr/bin/env Rscript
# ------------------------------------------------------------------------------
# Filename:    4_appendix_filters.R
# Description: Fits wordfish-all-political-dla
# Source:      Slapin and Proksch (2008); based on Wordfish v1.3
# ------------------------------------------------------------------------------

# Iterate over user types, elections and filter conditions

for(tweeters_to_analyze in c('users','politicians'))
{

if(tweeters_to_analyze == 'politicians'){
	mode <- 'wf'
	
	# Candidate option
	min_words_per_tweeter_party <- 30 # a user needs to tweet enough 
	in_min_docs_users           <- 3 # a word needs to belong to enough users 
	...
	# User option
	in_min_docs_users           <- 10 # a word needs to belong to enough users (example: 4 or 3)
	
	# Candidates options
	min_words_per_tweeter_party <- 25 # a user needs to tweet enough 
	in_min_docs_party           <- 10  # a word needs to belong to enough users 
...
# Global parameters
link_type                  <- 'text' # text (or hashtags - obsolete)
chosen_dim                 <- ''    # ca-option, put at '' for wordfish-wgt
ngram                      <- '2gram' # "ngram", choose n
...
for(election in c('2014-can-quebec','2015-can-canada','2014-nzl-new-zealand'))

{
	
TM_file_users        <- paste0(input_path,election,'/TM',ngram,'.csv')
TM_file_party        <- paste0(input_path,election,'/politicians_TM',ngram,'.csv')
tweeters_file_users  <- paste0(input_path,election,'/link.csv')
tweeters_file_party  <- paste0(input_path,election,'/politicians_link.csv')
file_init            <- paste0(input_path,election,'/politicians_init.csv')


## Output files
path        <- paste0('../data/filters/ideologies/',election,'/')
...
# b) Wordfish (fixed beta)
wordfish2<- function(words_weight,input,wordsincol=FALSE,fixtwo=FALSE,dir=NULL,fixdoc=c(1,2,0,1),tol=1e-7,sigma=3,boots=FALSE,nsim=500,writeout=FALSE,output="wordfish_output") {
    dta<-input
	...
	if(fixtwo==FALSE){
	      ...
	      diffllik<-500
        	...
        	while (diffllik>conv) { # Run algorithm if difference in LL > convergence criterion
            omegaprev<-params$omega
            bprev<-words_weight # COCO MODIF params$b
            alphaprev<-params$alpha
            psiprev<-params$psi
            ...
            resa <- optim(p=c(params$omega[1]),
                        fn=llik_alpha_1,
                        y=as.numeric(dta[1,]),
                        b= words_weight, # COCO MODIF params$b,
                        psi=params$psi,
                        method=c("BFGS")
                        )
                params$omega[1] <- resa$par[1]
          ...
          # Estimate all other omegas and alphas
            ...
            flush.console()
           	...
           	# Z-score transformation of estimates for omega (to identify model)
           	omegabar     <- mean(params$omega)
           	b1       <- words_weight # COCO MODIF params$b
           	# COCO MODIF params$b <- params$b * sd(params$omega)
           	params$omega <- (params$omega - omegabar)/ sd(params$omega)
           	params$psi <- params$psi + b1*omegabar
        	...
        	cat("\tUpdating psi and beta..\n")}
              ...
              for (j in 1:W) {
                 resb <- optim(par=c(words_weight[j],params$psi[j]),
                       fn=llik_psi_b,
                       y=dta[,j],
                       omega=params$omega,
                       alpha=params$alpha,
                ...
                # COCO MODIF params$b[j] <- resb$par[1]
                params$psi[j] <- resb$par[2]
                params$min2[j] <- -1.00*resb$value
                ifelse(resa$convergence!=0,print("Warning: Optim Failed to Converge!"),NA)
                }
    ...
    rownames(output.documents)<-rownames(dta)
    colnames(output.documents)<-c("omega","alpha")
    #output.words<-cbind(words_weight,est$psi)    COCO MODIF
    #rownames(output.words) <-words               COCO MODIF
    #colnames(output.words) <-c("b","psi")       COCO MODIF
...
return(list(documents=output.documents,diffllik=est$diffllik,diffomega=est$diffparam,maxllik=est$maxllik,estimation=output.estimation,ci.documents=ci.documents,ci.words=ci.words))
...
if(include_users && mode != 'wgt' && mode != 'wgt2' ){
	...
	kept_tweeters_users <- (colSums(TM_users) > min_words_per_tweeter_users)
...
if( (election == '2015-can-canada') && (tweeters_to_analyze == 'politicians') ){max_filter <- 50} # Need to go somewhere further before observing a drop

for(iterator in seq(min_filter,max_filter))
{
	### 1 Determine target
	if(tweeters_to_analyze == 'politicians'){in_min_docs_party <- iterator}
	else{min_words_per_tweeter_users <- iterator}

	### 2- Filter PARTY DATA
	if(include_party){
		# Load tables
		wordcountdata_party<- as.data.frame(data.table::fread(TM_file_party),stringsAsFactors=FALSE)
		tweeters_party<-read.csv(tweeters_file_party , stringsAsFactors=FALSE)
		...
		kept_tweeters_party <- (colSums(TM_party) > min_words_per_tweeter_party)
	...
	if(mix_tm){
		...
		min_words_per_tweeter <- c(min_words_per_tweeter_party,min_words_per_tweeter_party)
	}else if(tweeters_to_analyze == 'politicians' || (mode == 'wgt' || mode == 'wgt2') ){
		tweeters <- tweeters_party
		words    <- words_party
		TM       <- TM_party
		in_min_docs           <- in_min_docs_party
		min_words_per_tweeter <- min_words_per_tweeter_party
	}else if(tweeters_to_analyze == 'users'){
		tweeters <- tweeters_users
		words    <- words_users
		TM       <- TM_users
		in_min_docs           <- in_min_docs_users
		min_words_per_tweeter <- min_words_per_tweeter_users
	}else{
	...
	# a) Keep words that appear in enough users documents
	#kept_words_users <- (rowSums(TM > 0) > in_min_docs_users)
	#TM <- TM[kept_words_users,]
	#words <- words[kept_words_users]
	
	# b) Keep users with enough specific words
	#kept_tweeters_users <- (colSums(TM) > min_words_per_tweeter_users)
	#TM_users <- TM_users[,kept_tweeters_users]
	#tweeters_users <- tweeters_users[kept_tweeters_users]
	
	
	# RUN WORDFISH
	if(mode == 'wf' || (mode == 'wgt' || mode == 'wgt2')){
		wf_out <-wordfish(TM,fixtwo=FALSE,dir=c(1,2),wordsincol=FALSE,tol=1e-4)
		...
		stop('Problem with mode')
	}
	
	
	# Handle users weight using politicians information 
	if(tweeters_to_analyze == 'users' && (mode == 'wgt' || mode == 'wgt2') ){
		...
		non_zero_idx <- (colSums(TM_users_weighted != 0) > min_words_per_tweeter_users)
	...
	# Write tweeters file
	global_name <- paste0(link_type,'_',mode,chosen_dim,'_',ngram,'_',iterator,'_',tweeters_to_analyze,'.csv')

	tweeters_file_multiple      <- paste0(path,global_name)
	write.table(data.frame(tweeters,omega),file = tweeters_file_multiple,sep = ",",col.names = c('key','value'),row.names = F)

}
} # close loop over election
} # close loop over type of users
########################################################################################
